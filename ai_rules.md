...
*   **4.4. 優先使用免費的回覆訊息 (Prioritize Free Reply Messages)**:
    *   **成本考量**: 為了將營運成本降至最低，**應一律優先使用 `replyMessage()` (Reply API) 來回覆使用者操作**。
    *   **免費 vs. 付費**:
        *   `replyMessage()`: 使用有時效性的 `replyToken`，**完全免費**。
        *   `pushMessage()`: 隨時可主動推送，但**會產生費用**並消耗訊息額度。
    *   **實作模式**: 應遵循現有模式，將耗時的資料庫查詢等邏輯封裝在獨立函式中（例如 `getBookingHistoryMessage`），讓該函式**回傳**訊息物件，最後在主流程中統一使用 `replyMessage()` 進行回覆。
    *   **例外情況**: 只有在需要「主動」推播非使用者即時觸發的訊息時（例如：預約前一天的提醒、行銷活動通知），才應考慮使用 `pushMessage()`。
